...
RUN apt-get update -y && \
    apt-get install -qqy \
        nginx supervisor cron git curl links vim redis-tools mariadb-client locales openvpn \
        libssl-dev libicu-dev libfreetype6-dev libjpeg62-turbo-dev libpng-dev libpq-dev g++ libxml2-dev libxslt-dev
RUN echo "cs_CZ.UTF-8 UTF-8" > /etc/locale.gen && locale-gen cs_CZ.UTF-8 && \
...
RUN docker-php-ext-configure gd --with-freetype-dir=/usr/include/ --with-jpeg-dir=/usr/include/ && \
    docker-php-ext-install -j$(nproc) bcmath mbstring intl iconv zip mysqli pdo pdo_mysql opcache soap gd xsl
